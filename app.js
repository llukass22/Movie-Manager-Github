...
const imdbIds = Object.fromEntries(`End of Watch|2012::tt1855199
...
const posters = {
  'End of Watch|2012':'https://image.tmdb.org/t/p/w500/zTqDSbsF3TKoIc25S03esfuk14Q.jpg',
  'Civil War|2024':'https://m.media-amazon.com/images/M/MV5BNjJiM2Y0NmItYzEzZS00NDZhLTllYmQtOTg3OGRhMmNiY2UxXkEyXkFqcGc%40._V1_.jpg'
};
const posterCache = JSON.parse(localStorage.getItem('after-hours-posters') || '{}');
movies.forEach(movie => { const key = `${movie.title}|${movie.year}`; movie.poster = posters[key] ?? posterCache[key] ?? null; });
...
const removedIds = new Set(JSON.parse(localStorage.getItem('after-hours-removed') || '[]'));
const removedMovies = JSON.parse(localStorage.getItem('after-hours-removed-movies') || '[]');
...
function setTheme(dark) { document.body.classList.toggle('dark-mode', dark); themeToggle.textContent = dark ? 'Light mode' : 'Dark mode'; themeToggle.setAttribute('aria-pressed', String(dark)); localStorage.setItem('after-hours-theme', dark ? 'dark' : 'light'); }
